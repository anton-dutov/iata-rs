...
fn trailing_characters() {
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR), Err(Error::TrailingData));
}
...
fn unsupported_format() {
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR_S), Err(Error::InvalidFormatCode('S')));

    const PASS_STR_LITTLE_M: &str = "m1DESMARAIS/LUC       EABC123 YULFRAAC 0834 326J001A0025 100^100";
    assert_eq!(RawBcbp::from_str(PASS_STR_LITTLE_M), Err(Error::InvalidFormatCode('m')));
}
...
fn invalid_characters() {
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR), Err(Error::InvalidCharacters));
    const PASS_STR_MINIMAL: &str = "ç";
    assert_eq!(RawBcbp::from_str(PASS_STR_MINIMAL), Err(Error::InvalidCharacters));
}
...
fn invalid_start_of_security_data() {
    ...
    const PASS_STR: &str = "M1DESMARAIS/LUC       EABC123 YULFRAAC 0834 326J001A0025 100+100";
    assert_eq!(RawBcbp::from_str(PASS_STR), Err(Error::InvalidPrefix(Field::BeginningOfSecurityData, '+')));
}
...
fn invalid_start_of_version_number() {
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR), Err(Error::InvalidPrefix(Field::BeginningOfVersionNumber, '+')));
}
...
fn expected_integer() {
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR_1), Err(Error::ExpectedInteger(Field::NumberOfLegsEncoded)));
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR_2), Err(Error::ExpectedInteger(Field::LengthOfSecurityData)));
}
...
fn subsection_too_long() {
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR), Err(Error::SubsectionTooLong));
}
...
fn unexpected_end_of_input() {
    ...
    assert_eq!(RawBcbp::from_str(PASS_STR_SEC), Err(Error::UnexpectedEndOfInput(Field::SecurityData)));

    // This is an incomplete type M pass truncated half way through the name field.
    const PASS_STR_NAME: &str = "M2DESMARAIS";
    assert_eq!(RawBcbp::from_str(PASS_STR_NAME), Err(Error::UnexpectedEndOfInput(Field::PassengerName)));
}
